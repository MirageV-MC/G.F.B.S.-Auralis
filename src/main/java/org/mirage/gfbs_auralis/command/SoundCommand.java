package org.mirage.gfbs_auralis.command;
...
public final class SoundCommand {
                                                                                                        ...
                                                                                                        .then(Commands.argument("min-distance", FloatArgumentType.floatArg(0.1f, 1000.0f))
                                                                                                                .then(Commands.argument("max-distance", FloatArgumentType.floatArg(0.1f, 1000.0f))
                                                                                                                        ...
                                                                                                                        .then(Commands.argument("targets", EntityArgument.players())
                                                                                                                                .executes(ctx -> playSound(ctx, EntityArgument.getPlayers(ctx, "targets"))))))))))))))))
    ...
    private static int playSound(CommandContext<CommandSourceStack> ctx, Collection<ServerPlayer> explicitTargets) throws CommandSyntaxException {
        ...
        ctx.getSource().sendSuccess(
                () -> Component.literal("[GFBS Auralis] 已向 " + finalSent + " 名玩家发送播放指令: " + soundEventId + " (id=" + id + ")"),
                false
        );
        return 1;
    }
